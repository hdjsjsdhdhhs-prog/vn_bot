...
func TestMiniAppRecent_RecoveryAndOwnership(t *testing.T) {
 ...
 read := func(id int64) []service.PurchaseOrderInfo {
  ...
  var result struct { Orders []service.PurchaseOrderInfo `json:"orders"` }
 ...
 require.NoError(t, db.GetDB().Model(&database.Order{}).Where("purchase_id = ?", order.OrderID).
  Updates(map[string]any{"purchase_expires_at": past, "stars_checkout_id": "private-checkout"}).Error)
...
func TestMiniAppRecent_BoundedNewestFirst(t *testing.T) {
 ...
 for i := 0; i < 23; i++ {
  ...
  key := uuid.NewString()
  order := &database.Order{PurchaseID: &ref, PurchaseKey: &key, BuyerTelegramID: &sub.TelegramID,
 ...
 for i := 1; i < len(result.Orders); i++ { assert.False(t, result.Orders[i].CreatedAt.After(result.Orders[i-1].CreatedAt)) }
}
